...
def load_pipeline(pipeline_name: str, hunyuan_path: str, device: str = "cuda", dtype_str: str = "bfloat16"):
    ...
    try:
        logger.info(f"Loading Hunyuan Video pipeline from: {hunyuan_path}")
        # Use DiffusionPipeline to handle either T2V or I2V variants seamlessly
        base_pipe = DiffusionPipeline.from_pretrained(hunyuan_path, torch_dtype=torch_dtype, local_files_only=True)
        ...
        logger.error(f"Hunyuan load failed: {exc}")
