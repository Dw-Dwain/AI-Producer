...
Phase 7: 🎬 Professional Production Workflows
"""
...
def create_video_tab(db: DatabaseManager):
            ...
            with gr.TabItem("🎬 Workflow Builder"):
                with gr.Row():
                    with gr.Column(scale=2, variant="panel"):
                        gr.Markdown("#### 🎭 Context & Shot Setup")
                        with gr.Row():
                        ...
                        vid_ref_image = gr.Textbox(label="Reference / Starting Frame (path)", placeholder="/path/to/image.png", interactive=True)
                    ...
                    with gr.Column(scale=1, variant="panel"):
                        ...
                        btn_queue_render = gr.Button("🚀 Add to Render Queue", variant="primary")
                        vid_action_msg = gr.Markdown("")

            with gr.TabItem("⚙️ Render Queue"):
                gr.Markdown("#### Background Render Jobs")
                with gr.Row():
                    btn_refresh_queue = gr.Button("🔄 Refresh Queue", variant="secondary")
                ...
                with gr.Row():
                    btn_q_cancel  = gr.Button("🛑 Cancel Job", variant="stop")
                    btn_q_approve = gr.Button("✅ Approve (Store)", variant="primary")
    ...
    def _job_row(job: dict) -> list:
        return [job["id"], job.get("status", "unknown"), job.get("model_family", ""), job.get("character_name", "—"), job.get("shot_type_name", "—"), (job.get("created_at") or "")[:16]]

    def _meta_md(vid: dict, is_job=False) -> str:
        if not vid: return "*No selection.*"
        ...
        if is_job:
            ...
            lines.append(f"**Video ID:** {vid['id']}  ·  **Family:** `{vid.get('model_family','LTX-Video')}`  ·  **Pipeline:** `{vid.get('pipeline','?')}`")
    ...
    def on_detect_ltx(ltx, gemma, upscaler, lora_dir):
        ...
        return _status_html(result, ltx_manager.get_loaded_pipeline_name())
    ...
    def on_queue_render(prompt, neg_prompt, family, pipeline_name, preset_val, w, h, fps, nf, steps, cfg, lora_dir, lora_sel, lora_wt, seed, ref_img, proj, char, loc, shot):
        ...
        char_id = db.get_character_by_name(char)["id"] if char and char != "All" else None
        loc_id = db.get_location_by_name(loc)["id"] if loc and loc != "All" else None
        shot_id = db.get_shot_template_by_name(shot)["id"] if shot else None
        
        actual_seed = int(seed) if int(seed) != -1 else random.randint(0, 2**32 - 1)
        lora_full = os.path.join(lora_dir, lora_sel) if lora_sel and lora_sel != "None" and lora_dir else ""

        job_id = db.add_to_render_queue(
            project_id=proj_id, scene_id=None, character_id=char_id, location_id=loc_id, shot_type_id=shot_id,
            model_family=family, pipeline=pipeline_name, preset=preset_val, prompt=prompt, negative_prompt=neg_prompt,
            seed=actual_seed, width=int(w), height=int(h), fps=int(fps), num_frames=int(nf), steps=int(steps),
            guidance_scale=float(cfg), lora_path=lora_full, lora_weight=float(lora_wt), reference_image_path=ref_img
        )
        return f"✅ **Job #{job_id} Queued!** Background worker will process it."
    ...
    def on_q_approve(sel):
        if sel == -1: return "⚠️ Select a job."
        job = db.get_render_job(int(sel))
        if job["status"] == "completed" and job.get("video_id"):
    ...
    def on_lib_refresh(proj_f, char_f, loc_f, date_f, fam_f, pipe_f):
        ...
        loc_id = db.get_location_by_name(loc_f)["id"] if loc_f and loc_f != "All" else None
        
        all_vids = db.list_generated_videos(project_id=proj_id, character_id=char_id, location_id=loc_id, status="approved", pipeline=pipe_f if pipe_f != "All" else None, model_family=fam_f if fam_f != "All" else None)
        ...
        if date_f != "All Time":
            ...
            now = datetime.utcnow()
            for v in all_vids:
                if not v.get("created_at"): continue
                try:
    ...
    btn_lib_refresh.click(on_lib_refresh, [lib_filter_proj, lib_filter_char, lib_filter_loc, lib_filter_date, lib_filter_family, lib_filter_pipeline], [lib_gallery, lib_action_msg, lib_vid_player, lib_detail_md])
